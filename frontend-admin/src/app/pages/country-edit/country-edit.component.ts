...
export class CountryEditComponent implements OnInit, HasUnsavedChanges {
  ...
  addAlias(): void {
    ...
    if (!alias || !this.countryId) return;

    this.addingAlias.set(true);
    this.countriesService
      .addAlias(this.countryId, alias)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          this.newAlias.set('');
  ...
  confirmDeleteAlias(alias: CountryAlias): void {
    ...
    dialogRef
      ...
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((confirmed) => {
        if (confirmed && this.countryId) {
          this.countriesService
            .removeAlias(this.countryId, alias.id)
            .pipe(takeUntilDestroyed(this.destroyRef))
            .subscribe({
              next: () => {
  ...
  private get ConfirmDialogComponent() {
    return ConfirmDeleteAliasDialogComponent;
  }
...
@Component({
  selector: 'app-confirm-delete-alias-dialog',
...
export class ConfirmDeleteAliasDialogComponent {
