...
router.get('/api/cities', async (req: Request, res: Response) => {
  const { country_id, search, page, limit, sortBy, sort, includeDisabled } = req.query;
  ...
  try {
    const result = await cityService.getCities({ country_id: countryIdNum, search: searchStr, page: pageNum, limit: limitNum, sortBy: sortByStr, sort: sortOrderStr, includeDisabled: includeDisabled === 'true' });
